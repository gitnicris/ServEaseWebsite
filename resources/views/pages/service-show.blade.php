...
<div class="max-w-5xl mx-auto px-6 py-10 text-white">
    <div class="bg-black/40 backdrop-blur-lg border border-white/10 rounded-2xl shadow-xl p-8">
        ...
        <div>
            ...
            @if ($provider && $profile)
                <!-- Whole provider card is clickable -->
                   ...
                   class="group flex flex-col md:flex-row md:items-center md:space-x-5 cursor-pointer">
                    ...
                    <div class="mb-4 md:mb-0">
                        @if ($profile->photo && Storage::disk('public')->exists($profile->photo))
                            <img src="{{ asset('storage/' . $profile->photo) }}"
                                 class="w-20 h-20 rounded-full object-cover border border-white/20 shadow-md transition-transform duration-200 group-hover:scale-105">
                        @else
                            <div class="w-20 h-20 rounded-full bg-gray-700 flex items-center justify-center text-gray-400 text-2xl border border-white/10 transition-transform duration-200 group-hover:scale-105">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-2">
                            <p class="font-bold text-xl group-hover:text-orange-300 transition-colors">
                                {{ $provider->name }}
                            </p>
                            <span class="text-[11px] uppercase tracking-wide bg-white/10 px-2 py-0.5 rounded-full text-gray-200">
                                View full profile
                            </span>
                        </div>
                        ...
                        <div class="grid sm:grid-cols-2 gap-3 mt-4 text-sm text-gray-300">
                            <div class="flex items-center gap-2">
                                <i class="bi bi-geo-alt text-orange-300"></i>
                                <span>{{ $profile->address ?: 'Address not provided' }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <i class="bi bi-telephone text-orange-300"></i>
                                <span>{{ $profile->phone ?: 'Phone not provided' }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <i class="bi bi-envelope text-orange-300"></i>
                                <span>{{ $profile->gmail ?: 'Email not provided' }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <i class="bi bi-info-circle text-orange-300"></i>
                                <span>
                                    {{ $profile->about ? \Illuminate\Support\Str::limit($profile->about, 70) : 'No additional info yet.' }}
                                </span>
                            </div>
                        </div>
                    </div>
